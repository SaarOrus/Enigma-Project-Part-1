...
public class CheckXMLFile {
    ...
    private static final int MAX_ROTOR_COUNT=99;
    // check valid xml function
    //ctor
    public CheckXMLFile(CTEEnigma cteEnigma) throws ReflectCharToTheSameCharException, DoubleMappingInRotorException, ReflectorIdIsMissingException, ReflectorIdIsNotUniqueException, ReflectorIdIsOutOfRangeException, NotchPlaceOutOfRangeException, RotorIdIsNotUniqueException, RotorIdIsMissingException, RotorIdOutOfRangeException, NumberOfCharactersNotEvenException, NumberOfRotorsIsNotValidException, NumberOfRotorsIsLessThenTwoException, NumberOfCharIsZeroException, RotorsCountBiggerThenMax {
        this.machine = cteEnigma.getCTEMachine();
        checkXmlExceptions();
    }

    public void checkXmlExceptions() throws NumberOfRotorsIsNotValidException, NumberOfCharactersNotEvenException, NumberOfRotorsIsLessThenTwoException, ReflectCharToTheSameCharException, ReflectorIdIsMissingException, ReflectorIdIsNotUniqueException, ReflectorIdIsOutOfRangeException, NotchPlaceOutOfRangeException, RotorIdOutOfRangeException, RotorIdIsNotUniqueException, RotorIdIsMissingException, DoubleMappingInRotorException, NumberOfCharIsZeroException, RotorsCountBiggerThenMax {
        NumberOfCharIsZero();
        isNumberOfCharactersEven();
        isRotorCountSmallerThenNumbersOfRotors();
    ...
    public void isRotorCountSmallerThenNumbersOfRotors() throws NumberOfRotorsIsNotValidException {
        if (machine.getRotorsCount() > machine.getCTERotors().getCTERotor().size())
            throw new NumberOfRotorsIsNotValidException(machine.getRotorsCount(),machine.getCTERotors().getCTERotor().size());

    }
    public void isRotorCountBiggerThenMaximum() throws RotorsCountBiggerThenMax {
        if (machine.getRotorsCount() > MAX_ROTOR_COUNT)
            throw new RotorsCountBiggerThenMax(machine.getRotorsCount());
